...
function App() {
  ...
  const completeTodoRequest = async (todo) => {
    try {
      const { data } = await axios.patch(`${API_URL}/todos/${todo.id}`, {
              ...
              {todos.map((todo) => (
                <li key={todo.id}>
                  ...
                  <input
                    ...
                    type="checkbox"
                  />
                  <button onClick={() => deleteTodoRequest(todo)}>
                    удалить
                  </button>
                  <button onClick={() => changeTodoRequest(todo)}>
                      {change ? 'изменить' : 'применить'}
                    </button>
                  {/* {!change && 
                    <button onClick={setChange(!change)}>применить</button>
                  }
                  {change && 
                    <button onClick={() => changeTodoRequest(todo)}>
                      изменить
